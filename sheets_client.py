...
class SheetsClient:
    ...
    def _get_all_claims_sync(self, worksheet: str) -> List[List]:
        """Get all claim rows from worksheet, skipping header."""
        service = self._get_service()
        try:
            result = service.spreadsheets().values().get(
                spreadsheetId=self.spreadsheet_id,
            ...
            logger.error(f"Error getting claims from {worksheet}: {e}")
            return []

    def _delete_row_sync(self, worksheet: str, row_index: int) -> bool:
        """Delete specific row by index (1-based, including header)."""
        service = self._get_service()
        try:
            ...
            logger.error(f"Error deleting row {row_index} from {worksheet}: {e}")
            return False

    def _get_all_users_sync(self, worksheet: str) -> List[List]:
        """Get all user rows from worksheet, skipping header."""
        service = self._get_service()
        try:
            result = service.spreadsheets().values().get(
                spreadsheetId=self.spreadsheet_id,
            ...
            values = result.get('values', [])
            return values[1:] if values else []  # Skip header
        except Exception as e:
            logger.error(f"Error getting users from {worksheet}: {e}")
            return []

    def _delete_user_row_sync(self, worksheet: str, user_id: int) -> bool:
        """Delete row containing the user_id."""
        service = self._get_service()
        try:
            values = self._get_all_users_sync(worksheet)
                        ...
                        'requests': [{
                            'deleteDimension': {
                                'range': {
                                    'sheetId': sheet_id,
                                    'dimension': 'ROWS',
                                    'startIndex': i + 1,  # +1 for header, +1 for 0-index
                                    'endIndex': i + 2
                                }
                            }
                        }]
            ...
            logger.error(f"Error deleting user {user_id} from {worksheet}: {e}")
            return False
